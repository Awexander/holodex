...
async def test_playlist_artist(musicdex: MusicdexClient):
    response = await musicdex.playlist(category="artist", channel_id='UCoSrY_IQQVpmIRZ9Xf-y93g')
    await assert_playlist(response=response)
    
@pytest.mark.asyncio
async def test_playlist_dailyrandom(musicdex: MusicdexClient):
    response = await musicdex.playlist(category="dailyrandom", channel_id='UCoSrY_IQQVpmIRZ9Xf-y93g')
    await assert_playlist(response=response)
